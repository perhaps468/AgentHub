...
> 本文档用于把 AgentHub 当前项目能力整理为适合比赛交付的产品设计与实现分期文档。
>
> 文档目标不是复述所有底层代码细节，而是以产品视角说明系统为什么这样分期、每一期解决什么问题、采用什么核心接口与实施方案、做到什么程度算完成，以及当前明确不纳入本次交付的边界。
...
- 不把长期自治、周期唤醒、长期记忆写成本次已交付能力
- 不把完整企业级权限系统、租户系统、组织系统写入本次范围
- 不把复杂 Browser Use、Computer Use、云容器平台作为本次基础能力
- 不把完整 Marketplace、公开分享生态作为本次必须完成的交付目标
...
- 后端提供 `PATCH /api/sessions/{session_id}` 更新接口，支持修改 `title`、`is_pinned`、`is_archived` 等字段；前端通过会话列表菜单或会话头部入口调用该接口。
...
### P1-5 Phase1 验收与前端体验闭环

**目标**

确认系统已经是一个稳定的聊天底座，而不是单次演示页面。

**接口/实施方案**

- 主链路验收按 `POST /api/sessions -> GET /api/sessions -> WS /ws/{session_id} -> send_message -> GET /api/sessions/{session_id}/messages` 顺序执行，确保前后端流程可串起来。
- 验证消息结构、流式协议、会话排序规则和用户隔离规则都已经固定下来，可直接作为后续 Runtime 的前置约束。
- 检查前端 session store、message store 与后端接口结构是否一致，防止出现“页面能看见、接口却无法恢复”的旁路状态。
- 在文档层明确：Phase1 的消息模型、WS 协议和用户隔离规则进入 Phase2 后只能增强，不能推翻。

**测试/完成标准**

- IM 主链路闭环成立
- 消息流可作为后续 Runtime 统一输出层
- 用户隔离与历史恢复能力稳定可复用

**本阶段不做什么**

- 不在 Phase1 内实现真正的 Agent 推理闭环
