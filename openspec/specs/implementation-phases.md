...
> 文档目标不是提前做好所有底层代码细节，而是以产品视角说明系统为什么这样分期、每一期解决什么问题、采用什么核心接口与实施方案、做到什么程度算完成。文档定位也不是直接作为实现说明书，而是作为后续拆分各阶段 task 与校准实现边界的基准。
...
- Runtime 提供统一执行入口，当前实现以 `RuntimeAgentService` 作为运行时桥接层，由 WebSocket 收到用户消息后触发，再接入底层 `ReactAgent` / `LLMAdapter` 执行，而不是继续沿用直接拼接 UI 回复的临时链路。
- Runtime 负责读取会话历史、当前 Agent 配置、工作区绑定和上下文，再调用对应 Provider 的生成或流式生成能力。
- Runtime 负责把输出结果写回 `Message` 存储层，并通过统一事件桥接层推送到 `WS /ws/{session_id}`。
- WebSocket 层当前已主要承担连接管理、消息入站、运行时触发与事件出站职责；群聊编排与恢复逻辑也已在同一链路中接入，因此实际分层比文档抽象更紧一些。
...
- 建立 `PromptBuilder.build(...)` 或等价方法，按 `system`、`role`、`task`、`history`、`tool_instructions`、`memory_stub` 等模块拼接最终输入；当前实现以运行时 prompt 模板、工具摘要和环境注入为主。
...
- Prompt 组装层应支持注入当前会话输入、运行环境、工具说明和必要元数据，为后续任务拆解、代码任务和群聊主持能力提供统一入口。
...
- 建立 `ContextBuilder.build(session_id, trigger_message_id, agent_id)` 或等价流程，统一收集系统指令、最近消息、必要元数据和工具结果；当前实现以会话历史加载、消息过滤和上下文清洗为主。
- 上下文至少区分 `system_context`、`recent_messages`、`tool_results`、`session_metadata` 四个部分，避免后续所有数据都混在单一数组里；当前代码里这些边界更多体现在运行时内存、工具事件和会话绑定数据上。
...
- 定义 `ToolRegistry.register()`、`ToolRegistry.get()`、`ToolRegistry.execute()` 等基础接口，所有工具统一在注册表中管理；当前实现以 `ToolManager` 统一管理和执行工具集合。
- 单个 `Tool` 至少包含 `name`、`description`、`parameters_schema`、`execute(args)` 四部分，便于模型理解和 Runtime 执行。
- 模型输出若包含工具调用意图，Runtime 负责解析成结构化 `tool_call`，再交给统一工具管理层执行。
- 工具执行结果统一封装为 `tool_result`，写入当前运行上下文，必要时也可写成系统消息或运行事件返回前端。
- 第一批工具以基础读写、检索、环境查询、命令执行和变更确认相关工具为主，为 Phase3 的工作区、文件和执行环境做前置铺垫。
...
- Runtime 内部采用 ReAct 风格的循环结构，每一轮都要记录当前状态，而不是单纯写成一个长函数；当前实现更多通过事件桥和 agent loop 体现，而不是一个完全独立的中心化状态机类。
- 定义运行时状态，如 `thinking`、工具调用中、响应中、完成、错误等，供日志、审计和前端展示复用。
- 事件桥接层至少支持 `runtime_state`、`tool_event`、`message_delta`、`message_end`、`message_error` 等事件，并统一映射回 WebSocket。
...
- 对超时、空响应、解析失败、流式中断、工具执行失败等场景定义明确处理分支，至少返回错误事件或兼容回复。
- 当流式输出失败或模型结果异常时，至少保证消息状态、错误事件和最终前端反馈保持一致，避免前端无反馈。
- 记录最小运行信息，如 `session_id`、`agent_id`、`stream_id`、`error_type`、运行时状态和工具调用记录等，为 Phase4 调度审计层做准备。
...
- 以“用户消息进入 WS -> RuntimeAgentService / Runtime -> Provider 调用 -> 流式事件 -> Message 落库 -> 历史恢复”为核心链路进行验收。
...
- 运行时解析工作区优先走正式绑定，必要时才使用显式参数或环境变量兼容 fallback 路径，避免执行目标不清晰。
...
- 在 Runtime 工具层提供 `read_file`、`write_file`、`list_dir`、`glob`、`grep`、`replace`、`diff` 等文件操作能力，统一走工具管理层调用。
- 定义结构化修改协议，优先以补丁、diff 或范围修改方式描述变更，而不是依赖整文件黑盒覆盖。
...
- `PendingChange` 至少包含 `change_id`、`session_id`、`message_id`、`stream_id`、`run_id`、`task_id`、`agent_id`、`path`、`operation`、`unified_diff`、`status`、`created_at` 等字段。
- WebSocket 或消息流中的变更消息应关联 `pending_change_id`，便于前端点击消息后读取完整 Diff 内容。
- 前端展示层支持显示文件路径、Diff 摘要、执行 Agent、所属任务和当前状态，保证单 Agent 与群聊编排产物复用同一展示模式；必要时还可和 run/task/message 建立回溯关系。
...
- 当前实现中，命令执行结果至少先通过结构化工具输出回流到 Runtime，再由统一消息流或工具事件返回前端；必要时可继续扩展为更细粒度的终端事件。
...
- 为可预览产物提供统一预览返回能力；当前实现以消息流事件、预览结果字段和特定产物链路回传为主，后续再逐步收敛为统一预览对象。
...
- 应用接口请求体至少包含 `change_id`，必要时可附带 `session_id`；应用成功后更新工作区内容、变更状态和相关消息展示。
- 拒绝接口请求体至少包含 `change_id`，当前实现以最小拒绝操作为主，可在后续继续补充拒绝原因等扩展字段。
- `PendingChange.status` 当前至少支持 `pending_confirmation`、`applied`、`rejected`、`failed`，并在消息流和查询接口中保持一致。
...
- Agent 字段至少包含 `id`、`name`、`role`、`description`、`provider`、`model`、`system_prompt`、`capability_tags`、`tool_permissions`、`is_active`、`owner_id`。
...
- Agent 配置中明确保存 `provider`、`model` 等路由字段，运行时由调度层根据 Agent 配置和服务端 settings 解析出最终 Provider 实例与模型。
- 调度层支持按 Agent 选择 Provider、Model，避免前端直接传入真实密钥。
- 当前阶段先建立 Provider 注册表、模型切换和统一调用入口；更复杂的多 key 路由与大规模 fallback 策略可在后续继续增强。
- 相关调用结果、错误原因和运行状态记录到基础审计与运行日志中，为后续并行会话和群聊编排提供基础。
...
- 会话对象中保存 `agent_id` 或等价字段，使每个会话在运行时能解析当前负责执行的 Agent。
- `GET /api/sessions/{session_id}` 返回会话详情时，当前至少返回会话绑定 Agent 标识，以及群聊场景下的成员信息与主处理关系。
...
- 对会话级活跃运行已保留查询或恢复接口，为前端刷新页面后恢复对应执行状态做准备。
...
- 调度层记录关键调用信息，包括 `session_id`、`agent_id`、`provider`、`model`、运行时状态、工具事件、错误信息和部分规划来源字段。
- 对超时、空响应、解析失败、流式失败、Provider 不可用等情况定义明确兼容处理路径，必要时返回错误事件或兼容回复。
- 群聊主 Agent 的计划拆解调用、单 Agent 普通调用和代码任务调用尽量复用同一批基础审计能力，便于横向追踪。
- 关键运行决策既记录到日志，也可在需要时通过系统消息或任务状态反馈给前端，提升可解释性。
- 调用审计在本阶段先形成基础链路和问题定位能力，再逐步补齐更完整的调度审计体系。
...
- 以“`GET /api/agents` 查看配置 -> `POST/PATCH /api/agents` 维护 Agent -> 会话绑定 Agent -> 运行时解析 Provider/Model -> 记录运行日志与基础审计”为主链路进行验收。
