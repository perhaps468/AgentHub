...
平台最终以用户账号作为数据归属边界。会话、消息历史、自建 Agent、产物状态、VFS 和部署记录都必须能归属到明确用户；分阶段策略上，`MVP` 与 `P1` 允许先使用单用户占位方案（如 `dev_user` 或显式 `owner_id`）跑通 IM 与多 Agent 主线，`P2` 再补齐最小正式注册登录闭环，并把数据归属收束到真实 `current_user`。
...
0. 用户账号与数据归属（基础边界）
   该系统最终需要支持最小正式用户注册与登录能力，使用账号身份隔离用户数据。分阶段上，`MVP` 与 `P1` 先允许单用户占位方案承接 IM 主线，`P2` 再补齐邮箱密码注册、密码哈希、登录签发 JWT、获取当前用户，以及基于当前用户隔离会话和消息；不要求邮箱验证、找回密码、OAuth、复杂权限系统或管理后台。
